...
Use repository root, Node from `.nvmrc`, `npm ci --include=dev --workspaces --include-workspace-root`, and root `npm run build`. Set the Cloudflare project/Worker name to match `himalayanatlas` in the configuration (or change that non-secret name in a reviewed commit). Use `npm run deploy:cloudflare` to publish a version, or `npm run upload:cloudflare` to upload an unpromoted version. Both invoke the configuration's `npm run release:check` build hook. **The default public export excludes 19 unresolved releases and can pass the publication gate. The full research export remains blocked.** The missing-directory configuration error is fixed; that does not override the documented rights blockers.

A plain `npm run build` produces the reduced public export. `npm run build:research` produces the complete local research export, which cannot pass the deployment gate. Neither command alone proves CI success. `versions upload` creates a version, not a production rollout. Never replace the root build with a direct Next command. Do not pass `--assets` or alternate configs to bypass release checks when publishing.

### Gated production automation

Recommended production path: GitHub's `Atlas quality` workflow. It runs complete checks, builds and tests the research profile on both static runtimes, then rebuilds and tests the reduced public profile through Cloudflare before retaining only that public artifact under the commit SHA. Its deployment job consumes that same artifact only after quality succeeds on a main push. It does not rebuild in the credential-bearing step.

Before enabling automation:

1. Use the validated reduced public export (or resolve all licensing blockers before restoring the complete dataset set) and require `Atlas quality / Quality and static browser tests` in the main ruleset. Enable pull-request review and prevent bypasses.
...
`check-static-export.mjs` rejects symlinks, native/server binaries, source maps, oversized files, missing public assets, changed public bytes, missing routes and exports over the free-plan 20,000-file/25-MiB-per-file limits. The complete research export was approximately 95.1 MiB across 1,780 files; the reduced public inventory is recorded in `out/release-inventory.json`. Static asset requests are free under current Cloudflare pricing; paid features/custom-domain registration and future policy changes are outside the €0 hosting target.
...
After `npm run build`, run `npm run preview:cloudflare` and open `http://127.0.0.1:4174/atlas/`. This command generates a local-only config from the production routing settings and serves whichever profile was last built. It cannot upload, tunnel, or use remote bindings. `npm run test:e2e:public` tests the deployable profile, including omitted-file 404s, unavailable states and retained map/scientific features. To test all original features, run `npm run build:research` followed by `npm run test:e2e` and `npm run test:e2e:cloudflare`, then rebuild with `npm run build` before publishing.

After an authorized deployment, verify a clean browser session on the actual HTTPS hostname: Atlas initial fit/reset and boundary selection, retained optional layers, downstream trace, river-only scenarios, explicit unavailable states for omitted features, Sources/Methodology/licences, phone layout and keyboard controls. Confirm 404 status, gzip hashes, no runtime remote API calls and header/cache rules. Use `npm run profile:performance -- https://YOUR-ACTUAL-HOST /tmp/atlas-live-performance.json` and compare the declared budgets; never equate local software-rendered timing with live delivery measurements. No live-domain acceptance or DNS/HTTPS validation has been claimed in this branch.
...
## Reduced public export after the 18:04 deployment failure

The second supplied Cloudflare log built successfully and stopped at the repository's publication-rights check. The fix keeps all source releases and reviews unchanged. The default root build now chooses the public profile, prevents dependent feature modules and excluded manifest imports from entering the bundles, removes every unresolved release directory from the output, and writes a deterministic inventory of every remaining file and SHA-256. No source data or published version is overwritten.
