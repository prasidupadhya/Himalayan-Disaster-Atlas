  ...
  return <div className="page">
    ...
    {Object.entries(ledger.reviews).map(([key, review]) => {
      ...
      return <section className="source-entry" key={key} id={key.replace('@', '-')}>
        ...
        {record ? <p>{record.attribution} <Link href={`/data-catalog/?dataset=${encodeURIComponent(record.id)}&version=${encodeURIComponent(record.version)}`}>Catalog and processing details</Link></p> : null}
        {notice ? <p><a href={notice}>Complete source notice</a></p> : null}
