...
import { WaterComparisonImages } from './water-comparison-images';
...
export function WaterChange({ map, temporal }: { map: Map | null; temporal?: TemporalSelection | null }) {
  const [resource, setResource] = useState<Resource<WaterManifest>>({ status: 'loading' });
  const [attempt, setAttempt] = useState(0);
  const [localBefore, setBefore] = useState('2024-04-19');
  const [localAfter, setAfter] = useState('2025-04-01');
  const [mode, setMode] = useState<Mode>('change');
  const [localVisible, setVisible] = useState(false);
  const [imageKey, setImageKey] = useState('');
  const [imageState, setImageState] = useState<Resource<{ url: string; path: string }>>({ status: 'empty', data: { url: '', path: '' } });
  const before = temporal?.before ?? localBefore;
  const after = temporal?.date ?? localAfter;
  const visible = temporal ? true : localVisible;
  ...
  useEffect(() => {
    ...
    void loadWaterArtifact(artifact, controller.signal).then(bytes => {
      ...
      setImageKey(artifact.path);
      setImageState({ status: 'ready', data: { url, path: artifact.path } });
      if (map) {
    ...
    }).catch(error => { if (!controller.signal.aborted) { setImageKey(artifact.path); setImageState({ status: error instanceof UnavailableError ? 'unavailable' : 'error', message: 'Water image could not be verified.' }); } });
    return () => {
      controller.abort();
  ...
  return <section className="thematic-controls" aria-label="Water Change" data-water-change-state={resource.status}>
    ...
    <label className="thematic-picker">Earlier water observation<select value={before} disabled={!manifest || Boolean(temporal)} onChange={e => { loading(); setBefore(e.target.value); }}>{temporal && !manifest?.observations.some(o => o.id === before) && <option value={before}>{before || 'No comparison date'} · unavailable</option>}{manifest?.observations.map(o => <option key={o.id}>{o.id}</option>)}</select></label>
    <label className="thematic-picker">Later water observation<select value={after} disabled={!manifest || Boolean(temporal)} onChange={e => { loading(); setAfter(e.target.value); }}>{temporal && !observation && <option value={after}>{after} · unavailable</option>}{manifest?.observations.map(o => <option key={o.id}>{o.id}</option>)}</select></label>
    ...
    <label><input type="checkbox" checked={visible} disabled={!manifest || Boolean(temporal)} onChange={e => { loading(); setVisible(e.target.checked); }} /> Show water observation</label>
    <button disabled={!map || !manifest} onClick={() => { if (map && manifest) { const [w, s, e, n] = manifest.metadata.spatial_coverage.bbox; map.fitBounds([[w, s], [e, n]], { padding: 45, maxZoom: 12, duration: 0 }); document.getElementById('atlas-map')?.scrollIntoView({ block: 'start' }); } }}>Zoom to Phewa window</button>
    ...
    {visible && artifact && <><DataState state={imageKey === artifact.path ? imageState : { status: 'loading' }} retry={() => { loading(); setAttempt(v => v + 1); }} />{'data' in imageState && imageState.data.url && imageState.data.path === artifact.path && <figure className="water-preview">{/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={imageState.data.url} alt={`${mode.replaceAll('_', ' ')} for ${after}${mode === 'change' ? ` compared with ${before}` : ''}`} width="505" height="512" /><figcaption>{mode === 'quality_preview' ? 'Grey: source exclusion; light grey: 20 m buffer; gold: uncertain; purple: below minimum mapping unit.' : mode === 'true_colour' ? 'Source true colour for visual inspection; clouds and shadows are not removed from imagery.' : 'Blue: mapped water; green: gain; orange: loss; grey: UNKNOWN; transparent: mapped land.'}</figcaption></figure>}</>}
    {temporal && manifest && !observation && <p role="status">UNAVAILABLE — no water observation on {after} UTC. No nearby scene is substituted.</p>}
    {temporal && manifest && before < after && observation && manifest.observations.some(o => o.id === before) && <WaterComparisonImages key={`${before}/${after}`} before={manifest.observations.find(o => o.id === before)!} after={observation} />}
