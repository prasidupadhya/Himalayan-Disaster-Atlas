...
export function Satellite({ map, temporal }: { map: Map | null; temporal?: { date: string } | null }) {
  const [resource, setResource] = useState<Resource<SatelliteDataset>>({ status: 'loading' });
  const [attempt, setAttempt] = useState(0); const [localSelected, setSelected] = useState('central'); const [localVisible, setVisible] = useState(false); const [opacity, setOpacity] = useState(.82);
  ...
  return <section className="thematic-controls" aria-label="Satellite" data-satellite-state={resource.status}><h2>Satellite</h2><p>Sentinel-2 L2A · spring 2026 observation windows</p>
    ...
    <label className="thematic-picker">Observation window<select value={selected} disabled={!dataset || Boolean(temporal)} onChange={event => setSelected(event.target.value)}>{temporal && !selected && <option value="">Unavailable for selected date</option>}{dataset?.observations.map(value => <option key={value.id} value={value.id}>{value.label} · {value.acquired_at.slice(0, 10)}</option>)}</select></label>
    <label><input type="checkbox" checked={visible} disabled={!dataset || Boolean(temporal)} onChange={event => setVisible(event.target.checked)} /> Show true-colour preview</label>
    ...
    {temporal && dataset && !item && <p role="status">UNAVAILABLE — no satellite observation on {temporal.date} UTC. No nearby date is substituted.</p>}
